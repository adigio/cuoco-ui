...
const generateIngredientsPDF = () => {
  ...
  mealPrep.ingredients.forEach((ingredient: Ingredient) => {
    ...
    const ingredientLine = `${quantity} ${unit} ${name}${isOptional ? " (opcional)" : ""}`.trim();

    doc.text(ingredientLine, 105, y, { align: "center" });
  ...
  const generateFullRecipePDF = () => {
    ...
    const recipes = mealPrep.recipes.map((r: Recipe) => r.name).join(" | ");
    ...
    mealPrep.ingredients.forEach((ingredient: Ingredient) => {
      const qtyText = `${ingredient.quantity || ""} ${ingredient.unit?.symbol || ""}`.trim();
      doc.text(qtyText, 190, yRight, { align: "right" });
      doc.text(ingredient.name || "", 135, yRight);
      yRight += 5;
