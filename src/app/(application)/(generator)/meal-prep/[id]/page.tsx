...
const handleDownloadPDF = () => {
  ...
  mealPrep.ingredients.forEach((ingredient) => {
    const quantity = (ingredient.quantity || "").toString();
    const unit = (ingredient.unit?.symbol || "").toString();
    const description = (ingredient.name || "").toString();

    // Descripción a la izquierda
    doc.text(description, 15, y);

    // Cantidad + unidad alineado a la derecha
  ...
  const handleDownloadFullPDF = () => {
    ...
    const recipes = mealPrep.recipes.map((recipe) => recipe.name).join(" | ");
    ...
    doc.setFontSize(10);
    mealPrep.steps.forEach((step, index) => {
      const text = `${index + 1}. ${step.title}: ${step.description}`;
      const lines = doc.splitTextToSize(text, 115); // 60% ancho
