...
export default function MealPrepResultsPage() {
  ...
  const [selectedMealPrep, setSelectedMealPrep] = useState<null | { id: number, name: string }>(null);
  ...
  return (
    ...
    <main className="flex-1">
      <div className="container mx-auto px-4 py-12">
        <h1 className="text-3xl font-bold text-center mb-2">Meal Preps sugeridos</h1>
        <p className="text-gray-600 text-center mb-8">
          Basados en {ingredients.filter(ing => ing.confirmed).length} ingredientes que tienes disponibles
        </p>
        <Container>
          {isLoading ? (
                    ...
                    <div className='flex justify-between items-center px-2 text-red-400'>
                      ...
                      <div className='flex items-center gap-3'>
                        <button 
                          ...
                          onClick={() => handleFavoriteClick({ id: mealPrep.id, name: mealPrep.title })}
                          disabled={isFavorite}
                        >
                          <FontAwesomeIcon 
                            className='w-4 h-4' 
